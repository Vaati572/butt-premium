...
import {
  ...
  Hash, Send, Pencil, Trash2, Copy,
  CheckCircle, Circle, AlertCircle, Eye, LayoutGrid,
  Upload, Download, Film, ImageIcon, XCircle,
} from "lucide-react"
...
const RESEAUX = {
  instagram: { label: "Instagram", bg: "bg-pink-500/15", border: "border-pink-500/40", text: "text-pink-400", emoji: "📸" },
  tiktok:    { label: "TikTok",    bg: "bg-zinc-500/15", border: "border-zinc-400/40", text: "text-zinc-300", emoji: "🎵" },
}
...
function UploadZone({ label, onUpload, uploading, currentUrl, currentType, currentName, onRemove, accept, compact }: {
  label?: string; onUpload: (f: File) => void; uploading: boolean
  currentUrl?: string; currentType?: string; currentName?: string
  onRemove: () => void; accept?: string; compact?: boolean
}) {
  ...
  if (currentUrl) {
    return (
      <div className="relative rounded-xl overflow-hidden border border-zinc-700 bg-zinc-900 group">
        {isVideo(currentType || "") ? (
          <video src={currentUrl} controls className={`w-full object-contain bg-black ${compact ? "max-h-28" : "max-h-52"}`} />
        ) : (
          <img src={currentUrl} alt="media" className={`w-full object-cover ${compact ? "h-28" : "max-h-52 object-contain"}`} />
        )}
        <div className="absolute top-2 right-2 flex gap-1.5 opacity-0 group-hover:opacity-100 transition-opacity">
          <a href={currentUrl} download target="_blank" rel="noopener noreferrer"
            className="w-7 h-7 bg-black/70 backdrop-blur rounded-lg flex items-center justify-center text-white hover:bg-black" title="Télécharger">
            <Download size={12} />
          </a>
          <button onClick={onRemove}
            className="w-7 h-7 bg-black/70 backdrop-blur rounded-lg flex items-center justify-center text-red-400 hover:bg-red-500/20" title="Supprimer">
            <XCircle size={12} />
          </button>
        </div>
        <div className="px-3 py-1.5 flex items-center gap-2 border-t border-zinc-800">
          {isVideo(currentType||"") ? <Film size={11} className="text-zinc-500 shrink-0"/> : <ImageIcon size={11} className="text-zinc-500 shrink-0"/>}
          <span className="text-zinc-500 text-[11px] truncate">{currentName || "Fichier uploadé"}</span>
          <button onClick={() => inputRef.current?.click()} className="ml-auto text-[11px] text-yellow-500 hover:text-yellow-400 font-semibold shrink-0">Remplacer</button>
        </div>
        <input ref={inputRef} type="file" accept={accept || "image/*,video/*"} className="hidden" onChange={handleChange} />
      </div>
    )
  }
  ...
  return (
    <div
      ...
      className={`border-2 border-dashed rounded-xl text-center cursor-pointer transition-all ${compact ? "p-3" : "p-5"} ${
      ...
      {uploading ? (
        <div className="flex flex-col items-center gap-1.5">
          ...
          <p className="text-zinc-400 text-xs">Upload en cours...</p>
        </div>
      ) : (
        <div className="flex flex-col items-center gap-1.5">
          <div className={`bg-zinc-800 rounded-xl flex items-center justify-center ${compact ? "w-8 h-8" : "w-10 h-10"}`}>
            <Upload size={compact ? 14 : 18} className="text-zinc-400" />
          </div>
          <p className="text-white text-xs font-semibold">{label || "Déposer un fichier"}</p>
          {!compact && <p className="text-zinc-500 text-[11px]">ou cliquer pour parcourir</p>}
        </div>
      )}
      <input ref={inputRef} type="file" accept={accept || "image/*,video/*"} className="hidden" onChange={handleChange} />
    </div>
...
function PublicationModal({ pub, date, societyId, profile, onClose, onSave }: {
  ...
  const [miniatureUrl, setMiniatureUrl] = useState(pub?.miniature_url || "")
  ...
  const uploadFile = async (file: File, folder: string) => {
    ...
    const path = `${societyId}/${folder}/${Date.now()}_${Math.random().toString(36).slice(2)}.${ext}`
    const { error } = await supabase.storage.from("publications").upload(path, file, { cacheControl: "3600", upsert: false })
  ...
  const handleUploadMedia = async (file: File) => {
    ...
    try {
      ...
      setFichierUrl(url); setFichierNom(file.name); setFichierType(file.type); setFichierTaille(file.size)
      if (file.type.startsWith("image/")) setImageUrl(url)
    } catch (err: any) { setUploadError("Erreur : " + (err.message || "bucket introuvable")) }
    setUploadingMedia(false)
  }

  const handleUploadThumb = async (file: File) => {
    setUploadingThumb(true); setUploadError("")
    try {
      const url = await uploadFile(file, "miniatures")
      setMiniatureUrl(url)
    } catch (err: any) { setUploadError("Erreur miniature : " + err.message) }
    setUploadingThumb(false)
  }

  const removeFile = async (url: string, setter: () => void) => {
    try {
      const path = url.split("/publications/")[1]
  ...
  const handleSave = async () => {
    ...
    const payload = {
      ...
      fichier_url: fichierUrl, fichier_nom: fichierNom,
      fichier_type: fichierType, fichier_taille: fichierTaille,
      miniature_url: miniatureUrl,
      reseau, statut,
      date_publication: datePub, heure_publication: heure,
      notes, updated_at: new Date().toISOString(),
    }
    if (pub?.id) await supabase.from("publications").update(payload).eq("id", pub.id)
    else          await supabase.from("publications").insert(payload)
    setSaving(false); onSave(); onClose()
  }

  const generateWithAI = async () => {
    const theme = aiThemeCustom || aiTheme; if (!theme) return
    setAiLoading(true)
    try {
      const res = await fetch("https://api.anthropic.com/v1/messages", {
        ...
        body: JSON.stringify({
          ...
          messages: [{ role: "user", content:
            `Tu es expert en marketing réseaux sociaux (${reseau === "instagram" ? "Instagram" : "TikTok"}).
Génère une publication sur le thème : "${theme}". Marque de cosmétiques naturels.
Réponds UNIQUEMENT en JSON sans markdown :
{"titre":"...","caption":"...","hashtags":"..."}
- titre : accrocheur, max 60 caractères
- caption : 2-4 paragraphes, emojis, adapté ${reseau}
- hashtags : 8-12 hashtags séparés par espaces, commençant par #`
          }]
        })
      })
      ...
      <div className="bg-[#111111] border border-zinc-800 rounded-2xl w-full max-w-2xl max-h-[92vh] overflow-hidden flex flex-col shadow-2xl">
        ...
        <div className="flex items-center justify-between px-6 py-4 border-b border-zinc-800 shrink-0">
          <div className="flex items-center gap-3">
            <div className={`w-9 h-9 rounded-xl flex items-center justify-center text-lg ${cfg.bg} border ${cfg.border}`}>{cfg.emoji}</div>
            <div>
        ...
        <div className="flex flex-1 overflow-hidden">
          ...
          <div className="flex-1 overflow-y-auto p-5 space-y-4">
                ...
                <div className="flex gap-2">
                  {(["instagram","tiktok"] as const).map(r => (
                    <button key={r} onClick={() => setReseau(r)}
                      ...
                      {RESEAUX[r].emoji} {RESEAUX[r].label}
                    </button>
                  ))}
                </div>
              </div>
            ...
            {/* Miniature + Media côte à côte */}
            <div className="grid grid-cols-2 gap-3">
              <div>
                <label className="block text-[11px] font-bold text-zinc-500 uppercase tracking-wider mb-2">
                  🖼️ Miniature <span className="text-zinc-600 font-normal normal-case">(calendrier)</span>
                </label>
                <UploadZone
                  label="Miniature"
                  ...
                  currentName="miniature"
                  onRemove={() => removeFile(miniatureUrl, () => setMiniatureUrl(""))}
                  accept="image/*"
                  compact
                />
              </div>
              <div>
                <label className="block text-[11px] font-bold text-zinc-500 uppercase tracking-wider mb-2">
                  📎 Fichier principal
                  {fichierNom && <span className="text-zinc-600 font-normal normal-case ml-1">({formatFileSize(fichierTaille)})</span>}
                </label>
                <UploadZone
                  label="Image / Vidéo"
                  onUpload={handleUploadMedia}
                  uploading={uploadingMedia}
                  currentUrl={fichierUrl}
                  currentType={fichierType}
                  currentName={fichierNom}
                  onRemove={() => removeFile(fichierUrl, () => { setFichierUrl(""); setFichierNom(""); setFichierType(""); setFichierTaille(0); if (imageUrl === fichierUrl) setImageUrl("") })}
                  compact
                />
              </div>
            </div>

            {/* URL externe si pas de fichier */}
            {!fichierUrl && (
              <div>
                <label className="block text-[11px] font-bold text-zinc-500 uppercase tracking-wider mb-2">🔗 URL externe (optionnel)</label>
                <input type="text" placeholder="https://..." value={imageUrl} onChange={e => setImageUrl(e.target.value)}
                  className="w-full bg-zinc-900 border border-zinc-800 rounded-xl px-3 py-2.5 text-sm text-white placeholder-zinc-600 focus:outline-none focus:border-zinc-600"/>
              </div>
            )}

            {uploadError && <p className="text-red-400 text-xs bg-red-500/10 border border-red-500/20 rounded-lg px-3 py-2">{uploadError}</p>}
          ...
          {/* Preview */}
          {showPreview && (
            <div className="w-64 border-l border-zinc-800 p-4 overflow-y-auto bg-[#0d0d0d] shrink-0">
              <p className="text-[11px] font-bold text-zinc-500 uppercase tracking-wider mb-3">
                Aperçu {reseau === "instagram" ? "📸" : "🎵"}
              </p>
              <div className={`rounded-2xl border overflow-hidden ${cfg.border} bg-zinc-900`}>
                {miniatureUrl ? (
                  <img src={miniatureUrl} alt="mini" className="w-full h-40 object-cover"/>
                ) : fichierUrl && !isVideo(fichierType) ? (
                  <img src={fichierUrl} alt="media" className="w-full h-40 object-cover"/>
                ) : imageUrl ? (
                  <img src={imageUrl} alt="url" className="w-full h-40 object-cover"
                ...
                <div className="p-3">
                  <div className="flex items-center gap-2 mb-1.5">
                    <span className={`text-[10px] font-bold ${cfg.text}`}>{cfg.emoji} {cfg.label}</span>
                    <span className={`ml-auto text-[10px] px-2 py-0.5 rounded-full ${STATUTS[statut].bg} ${STATUTS[statut].color} border ${STATUTS[statut].border}`}>
                      {STATUTS[statut].label}
                    </span>
                  </div>
                  {titre && <p className="text-white text-xs font-bold mb-1">{titre}</p>}
                  {caption && <p className="text-zinc-400 text-[11px] leading-relaxed line-clamp-4 whitespace-pre-wrap">{caption}</p>}
                  {hashtags && <p className={`text-[11px] mt-1.5 ${cfg.text} line-clamp-2`}>{hashtags.split(" ").slice(0,5).join(" ")}</p>}
                </div>
              </div>
            </div>
          )}
        ...
        <div className="px-6 py-4 border-t border-zinc-800 flex gap-3 shrink-0">
          <button onClick={handleSave} disabled={saving || !titre.trim() || uploadingMedia || uploadingThumb}
...
export default function PublicationModule({ activeSociety, profile }: Props) {
  ...
  const loadData = async () => {
    ...
    try {
      ...
      const { data } = await supabase.from("publications").select("*")
        .eq("society_id", activeSociety.id).gte("date_publication", start).lte("date_publication", end)
  ...
  const deletePub = async (pub: Publication) => {
    ...
    for (const url of [pub.fichier_url, pub.miniature_url].filter(Boolean)) {
      try {
        const path = url.split("/publications/")[1]
        if (path) await supabase.storage.from("publications").remove([path])
      } catch {}
  ...
  const dupliquerPub = async (pub: Publication) => {
    await supabase.from("publications").insert({
      ...
      miniature_url: pub.miniature_url,
      reseau: pub.reseau, statut: "brouillon",
      date_publication: pub.date_publication, heure_publication: pub.heure_publication,
      notes: pub.notes,
    })
    loadData()
  }
        ...
        {Array.from({ length: daysInMonth }).map((_, i) => {
          ...
          return (
            <div key={day}
              ...
              className={`rounded-xl border cursor-pointer transition-all group overflow-hidden flex flex-col ${
                isToday ? "border-yellow-500/60 bg-yellow-500/5" : isPast ? "border-zinc-800/50 bg-zinc-900/30" : "border-zinc-800 bg-zinc-900/50 hover:bg-zinc-900 hover:border-zinc-600"
              }`}
              style={{ minHeight: "90px" }}>

              {/* Numéro du jour */}
              <div className="px-2 pt-1.5 pb-1 flex items-center justify-between shrink-0">
                <span className={`text-xs font-bold ${isToday ? "text-yellow-500" : isPast ? "text-zinc-700" : "text-zinc-400"}`}>
                  {day}{isToday && <span className="ml-1 text-[9px]">●</span>}
                </span>
                {pubs.length === 0 && !isPast && (
                  <Plus size={11} className="text-zinc-700 opacity-0 group-hover:opacity-100 transition-opacity" />
                )}
              </div>

              {/* Publications */}
              <div className="flex-1 px-1 pb-1 space-y-1 overflow-hidden">
                {pubs.slice(0, 2).map(pub => {
                  const r = RESEAUX[pub.reseau]
                  const thumbSrc = pub.miniature_url || (pub.image_url && !isVideo(pub.fichier_type) ? pub.image_url : "")
                  return (
                    <div key={pub.id}
                      onClick={e => { e.stopPropagation(); setEditPub(pub); setShowModal(true) }}
                      className={`rounded-lg overflow-hidden border cursor-pointer hover:brightness-110 transition-all ${r.bg} ${r.border}`}>
                      {thumbSrc ? (
                        /* Miniature avec ratio fixe, rogné proprement */
                        <div className="w-full h-12 overflow-hidden relative">
                          <img src={thumbSrc} alt=""
                            className="absolute inset-0 w-full h-full object-cover" />
                          {/* Badge réseau en overlay */}
                          <div className={`absolute bottom-0.5 left-0.5 text-[9px] px-1 py-0.5 rounded font-bold ${r.bg} ${r.text} border ${r.border} backdrop-blur-sm`}>
                            {r.emoji}
                          </div>
                          {/* Statut dot */}
                          <div className={`absolute top-0.5 right-0.5 w-2 h-2 rounded-full ${STATUTS[pub.statut].color.replace("text-","bg-")}`} />
                        </div>
                      ) : (
                        /* Pas de miniature : affichage texte compact */
                        <div className="px-1.5 py-1">
                          <div className="flex items-center gap-1">
                            ...
                            <div className={`w-1.5 h-1.5 rounded-full shrink-0 ${STATUTS[pub.statut].color.replace("text-","bg-")}`} />
                          </div>
                        </div>
                      )}
                    </div>
                ...
                {pubs.length > 2 && (
                  <p className="text-zinc-600 text-[9px] text-center">+{pubs.length - 2}</p>
                )}
              </div>
            </div>
          )
            ...
            return (
              <div key={pub.id} className={`bg-zinc-900 border rounded-2xl p-4 hover:border-zinc-700 transition-all group ${s.border}`}>
                <div className="flex items-start gap-4">
                  {/* Miniature */}
                  <div className={`w-16 h-16 rounded-xl shrink-0 overflow-hidden border ${r.border} ${r.bg} flex items-center justify-center relative`}>
                    {thumbSrc ? (
                      <img src={thumbSrc} alt="" className="w-full h-full object-cover"/>
                    ) : isVideo(pub.fichier_type) ? (
                  ...
                  <div className="flex-1 min-w-0">
                    <div className="flex items-start justify-between gap-2 mb-1">
                      ...
                      <div className="text-right shrink-0">
                        ...
                        <p className="text-zinc-600 text-[11px]">⏰ {pub.heure_publication}</p>
                      </div>
                    </div>
                    ...
                    <div className="flex items-center gap-2 opacity-0 group-hover:opacity-100 transition-opacity">
                      ...
                      {pub.statut === "brouillon" && (
                        <button onClick={() => changerStatut(pub.id, "planifie")}
                          className="px-2 py-1 rounded-lg bg-yellow-500/10 border border-yellow-500/30 text-yellow-400 text-[11px] font-semibold hover:bg-yellow-500/20">
                          → Planifier
                        </button>
                      )}
                      {pub.statut === "planifie" && (
                        <button onClick={() => changerStatut(pub.id, "publie")}
                          className="px-2 py-1 rounded-lg bg-green-500/10 border border-green-500/30 text-green-400 text-[11px] font-semibold hover:bg-green-500/20">
                          ✓ Marquer publié
                        </button>
                      )}
