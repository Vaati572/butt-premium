...
interface CahierItem {
  ...
  semaine: string   // YYYY-MM-DD (lundi)
  done: boolean
  created_at: string
}
...
/* ── FORM CAHIER ─────────────────────────────── */
function CahierForm({ societyId, profile, item, semaine, onClose, onDone }: {
  societyId: string; profile: any; item?: CahierItem; semaine: string; onClose: ()=>void; onDone: ()=>void
}) {
  ...
  return (
    <div className="fixed inset-0 bg-black/60 backdrop-blur-sm flex items-center justify-center z-50">
      <div className="bg-[#111111] border border-zinc-800 rounded-2xl w-full max-w-md shadow-2xl">
        <div className="flex items-center justify-between p-5 border-b border-zinc-800">
          <h3 className="text-white font-bold">{item ? "Modifier la tâche" : "Ajouter une tâche"}</h3>
          <button onClick={onClose} className="text-zinc-500 hover:text-white"><X size={16}/></button>
        </div>
        <div className="p-5 space-y-4">
          <div>
            <label className="block text-[11px] font-semibold text-zinc-500 uppercase tracking-wider mb-1.5">Tâche *</label>
            <input value={titre} onChange={e=>setTitre(e.target.value)} placeholder="Ex: Préparer les commandes fournisseurs" autoFocus
              className="w-full bg-zinc-800 border border-zinc-700 rounded-xl px-4 py-2.5 text-sm text-white focus:outline-none focus:border-blue-500/60"/>
          </div>
          <div>
...
export default function AgendaModule({ activeSociety, profile }: Props) {
  ...
  // Semaine courante agenda
  const [weekStart, setWeekStart] = useState(() => getMondayOf(new Date()))
  // Semaine courante cahier
  const [cahierWeek, setCahierWeek] = useState(() => getMondayOf(new Date()))

  const todayStr = new Date().toISOString().slice(0,10)

  const cahierWeekStr = cahierWeek.toISOString().slice(0,10)
  ...
  // Cahier items pour la semaine sélectionnée
  const cahierWeekItems = cahier.filter(c => c.semaine === cahierWeekStr)
  const cahierDone      = cahierWeekItems.filter(c => c.done)
  const cahierTodo      = cahierWeekItems.filter(c => !c.done)

  // Progress
  const progress = cahierWeekItems.length > 0
    ? Math.round((cahierDone.length / cahierWeekItems.length) * 100)
    : 0
  ...
  const prevCahierWeek = () => setCahierWeek(d => { const n=new Date(d); n.setDate(n.getDate()-7); return n })
  const nextCahierWeek = () => setCahierWeek(d => { const n=new Date(d); n.setDate(n.getDate()+7); return n })
  const goCahierToday  = () => setCahierWeek(getMondayOf(new Date()))
  ...
  const getTacheStyle = (t: Tache) => {
    ...
    const top    = (hStart - 7) * 56
    const height = Math.max((hEnd-hStart)*56, 28)
    return { top, height }
  }
  ...
  const cahierWeekEnd = new Date(cahierWeek)
  cahierWeekEnd.setDate(cahierWeekEnd.getDate()+6)
  const isCurrentWeek = cahierWeekStr === getMondayOf(new Date()).toISOString().slice(0,10)
        ...
        {view==="cahier" && (
          <div className="space-y-5">
            ...
            <div className="flex items-center justify-between bg-zinc-900 border border-zinc-800 rounded-2xl px-5 py-4">
              <div className="flex items-center gap-2">
                <button onClick={prevCahierWeek} className="p-1.5 text-zinc-400 hover:text-white hover:bg-zinc-800 rounded-lg"><ChevronLeft size={16}/></button>
                <button onClick={goCahierToday} className="px-3 py-1 text-xs font-semibold text-zinc-300 bg-zinc-800 hover:bg-zinc-700 rounded-lg">Cette semaine</button>
                <button onClick={nextCahierWeek} className="p-1.5 text-zinc-400 hover:text-white hover:bg-zinc-800 rounded-lg"><ChevronRight size={16}/></button>
              </div>
              <div className="text-center">
                <p className="text-white font-bold text-sm">
                  Semaine du {cahierWeek.getDate()} {MONTH_LABELS[cahierWeek.getMonth()]} au {cahierWeekEnd.getDate()} {MONTH_LABELS[cahierWeekEnd.getMonth()]} {cahierWeek.getFullYear()}
                </p>
                {isCurrentWeek && <p className="text-blue-400 text-[10px] font-semibold mt-0.5">Semaine en cours</p>}
              </div>
              <div className="text-right">
                <p className="text-white font-bold text-sm">{cahierDone.length}/{cahierWeekItems.length}</p>
                <p className="text-zinc-500 text-[10px]">terminées</p>
              </div>
            </div>

            {/* Barre de progression */}
            {cahierWeekItems.length > 0 && (
              <div className="bg-zinc-900 border border-zinc-800 rounded-2xl px-5 py-4">
                <div className="flex items-center justify-between mb-2">
                  <p className="text-zinc-400 text-xs font-semibold">Progression de la semaine</p>
            ...
            <div>
              ...
              {cahierTodo.length === 0 ? (
                <div className="text-center py-14 bg-zinc-900 border border-zinc-800 rounded-2xl border-dashed">
                  ...
                  <p className="text-zinc-500 text-sm font-semibold">Aucune tâche cette semaine</p>
                  <p className="text-zinc-600 text-xs mt-1 mb-4">Planifie les tâches importantes à accomplir</p>
                  <button onClick={()=>{setEditCahierItem(null);setShowCahierForm(true)}}
                          ...
                          {items.map(item=>(
                            <div key={item.id}
                              ...
                              <div className="flex-1 min-w-0">
                                ...
                                {item.user_nom && (
                                  <p className="text-zinc-600 text-[10px] mt-1">par {item.user_nom}</p>
                                )}
                              </div>
            ...
            {cahierDone.length > 0 && (
              <div>
                <h2 className="text-zinc-600 font-bold text-sm mb-3">✓ Terminées cette semaine ({cahierDone.length})</h2>
                <div className="space-y-1.5">
                  {cahierDone.map(item=>(
