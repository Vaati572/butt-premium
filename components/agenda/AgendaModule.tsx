...
import { Plus, X, Pencil, Trash2, Check, Clock, ChevronLeft, ChevronRight } from "lucide-react"
...
export default function AgendaModule({ activeSociety, profile }: Props) {
  ...
  // Taches per day per hour
  const getTachesForSlot = (date: string, hour: number) =>
    taches.filter(t => t.date===date && parseInt(t.heure_debut.slice(0,2))===hour)

  // Position tache in week view
  const getTacheStyle = (t: Tache) => {
    ...
    const top    = (hStart - 7) * 56  // 56px per hour
            ...
            <div className="flex items-center gap-1 bg-zinc-900 border border-zinc-800 rounded-xl p-1">
              {(["week","list"] as const).map(v=>(
                <button key={v} onClick={()=>setView(v)}
                  className={`px-3 py-1.5 rounded-lg text-xs font-semibold transition-colors ${view===v?"bg-blue-500 text-white":"text-zinc-500 hover:text-zinc-300"}`}>
                ...
                <div className="space-y-2">
                  {upcomingTaches.map(t=>(
                    <div key={t.id} className={`flex items-center gap-3 bg-zinc-900 border rounded-xl px-4 py-3 hover:border-zinc-600 transition-colors group ${t.done?"opacity-50 border-zinc-800":"border-zinc-800"}`}>
                      <div className="w-3 h-full rounded-full shrink-0" style={{backgroundColor:t.couleur, width:4, minHeight:40, borderRadius:4}}/>
