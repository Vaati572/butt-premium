...
import { Plus, X, Eye, Trash2, Users, ChevronDown, ChevronUp } from "lucide-react"
...
export default function PlaylistsModule({ activeSociety, profile }: Props) {
  ...
  const save = async () => {
    ...
    setSaving(true)
    const { error } = await supabase.from("playlists_clients").insert({
      nom: nom.trim(),
    ...
    setSaving(false)
    setShowForm(false)
    setNom(""); setSelected(new Set()); setSearch("")
    load()
  }
  ...
  const getClientsForPlaylist = (pl: Playlist) =>
    clients.filter(c => pl.client_ids?.includes(c.id))
        ...
        <div className="fixed inset-0 bg-black/70 backdrop-blur-sm z-50 flex items-center justify-center p-4">
          <div className="bg-[#111111] border border-zinc-800 rounded-2xl w-full max-w-md shadow-2xl flex flex-col max-h-[90vh]">
            <div className="flex items-center justify-between p-5 border-b border-zinc-800 shrink-0">
              <h2 className="text-white font-bold">Nouvelle playlist</h2>
              <button onClick={() => { setShowForm(false); setNom(""); setSelected(new Set()); setSearch("") }}
            ...
            <div className="p-5 flex flex-col gap-4 flex-1 overflow-hidden">
              ...
              <button onClick={save} disabled={saving || !nom.trim() || selected.size === 0}
                ...
                {saving ? "Création..." : `Créer la playlist (${selected.size} client${selected.size > 1 ? "s" : ""})`}
              </button>
            </div>
          </div>
        </div>
