...
import { Save, Plus, Trash2, Check, Edit3, Target, StickyNote, ListTodo, Star } from "lucide-react"
...
export default function EspacePersoModule({ activeSociety, profile, targetIds, targetNom }: Props) {
  ...
  const [displayName, setDisplayName]   = useState(targetNom)
  const [avatarBg, setAvatarBg]         = useState("#eab308")
  ...
  const [newObjCible, setNewObjCible]   = useState("")
  const [newObjUnite, setNewObjUnite]   = useState("")
  ...
  const load = useCallback(async () => {
    ...
    // Chercher le profil pour avoir le nom/avatar
    const { data: foundProfiles } = await supabase
      .from("profiles")
      .select("id,nom,prenom,color")
      .in("id", targetIds)

    const found = foundProfiles?.[0]
    if (found) {
      setDisplayName(found.prenom || found.nom || targetNom)
      setAvatarBg(found.color || colors[targetNom.charCodeAt(0) % colors.length])
    }

    const profileId = found?.id || targetIds[0]
    ...
    const ventesJour = ventes.filter((v: any) => v.created_at?.slice(0, 10) === today)
    setStats({
    ...
    // Charger ou créer l'espace perso
    ...
    if (!espace) {
      const { data: created } = await supabase.from("espace_perso").insert({
        ...
        profile_nom: found?.prenom || found?.nom || targetNom,
    ...
    setLoading(false)
  }, [sid, targetIds, targetNom])

  useEffect(() => { load() }, [load])

  const saveAll = async (patch: Record<string, unknown>) => {
    if (!espaceId) return
  ...
  const addObjectif = async () => {
    ...
    const palette = ["#eab308","#22c55e","#3b82f6","#a855f7","#f97316"]
    const item: Objectif = {
      id: crypto.randomUUID(), label: newObjLabel.trim(),
      valeur_cible: Number(newObjCible) || 100, valeur_actuelle: 0,
      unite: newObjUnite || "%", color: palette[objectifs.length % palette.length]
    }
    const next = [...objectifs, item]
    setObjectifs(next)
    setNewObjLabel(""); setNewObjCible(""); setNewObjUnite(""); setShowObjForm(false)
    await saveAll({ objectifs: next })
  }

  const updateObjectifVal = async (id: string, val: number) => {
  ...
  return (
    <div className="flex-1 overflow-y-auto bg-[#0a0a0a]">
      ...
      <div className="border-b border-zinc-900 px-6 py-5 flex items-center gap-5"
        style={{ background: `linear-gradient(135deg, ${avatarBg}12, transparent)` }}>
        ...
        <div className="flex-1">
          <div className="flex items-center gap-2">
            ...
            <span className="flex items-center gap-1 text-[10px] font-bold px-2 py-0.5 rounded-full border"
              style={{ background: avatarBg + "20", borderColor: avatarBg + "50", color: avatarBg }}>
              <Star size={9} /> Espace perso
            </span>
          </div>
          <p className="text-zinc-500 text-sm mt-0.5">Notes · Todo · Objectifs · Post-its</p>
        </div>
        <div className="flex gap-3 shrink-0">
          {[
            ...
            <div key={s.label} className="text-right px-4 py-2.5 rounded-xl border border-zinc-800 bg-zinc-900/40">
              <p className="font-black text-xl leading-none" style={{ color: avatarBg }}>{s.value}</p>
      ...
      <div className="flex gap-0 border-b border-zinc-900 px-6">
        {([
          ...
          <button key={t.id} onClick={() => setActiveTab(t.id)}
            ...
            {t.icon} {t.label}
            {t.id === "todo" && todos.filter(x => !x.done).length > 0 && (
              <span className="text-[10px] px-1.5 py-0.5 rounded-full font-black"
                style={{ background: avatarBg + "25", color: avatarBg }}>
      ...
      <div className="px-6 py-5 max-w-4xl">
            ...
            <div className="flex items-center justify-between">
              ...
              {!editingNotes && (
                <button onClick={() => setEditingNotes(true)}
                  className="flex items-center gap-1.5 px-3 py-1.5 rounded-xl text-xs font-bold border border-zinc-800 text-zinc-400 hover:text-white hover:border-zinc-600 transition-colors">
            ...
            {editingNotes ? (
              <div className="space-y-2">
                <textarea value={notes} onChange={e => setNotes(e.target.value)} rows={16}
                  className="w-full bg-zinc-900 border border-zinc-700 rounded-2xl p-4 text-white text-sm outline-none focus:border-zinc-500 resize-none leading-relaxed"
              ...
              <div className="bg-zinc-900/40 border border-zinc-800 rounded-2xl p-5 min-h-[200px] cursor-pointer" onClick={() => setEditingNotes(true)}>
                {notes
                  ? <pre className="text-zinc-200 text-sm leading-relaxed whitespace-pre-wrap font-sans">{notes}</pre>
                  : <p className="text-zinc-600 text-sm italic">Cliquez pour ajouter des notes...</p>
                }
              </div>
            )}
          </div>
        ...
        {activeTab === "todo" && (
          <div className="space-y-3">
            <div className="flex items-center justify-between">
              <h2 className="text-white font-bold">✅ Liste de tâches</h2>
            ...
            <div className="flex gap-2">
              ...
              <button onClick={addTodo} className="px-4 py-2.5 rounded-xl text-sm font-bold text-black shrink-0"
                style={{ backgroundColor: avatarBg }}>
                <Plus size={15} />
              </button>
            </div>
            {todos.length === 0
              ? <p className="text-zinc-600 text-sm italic py-8 text-center">Aucune tâche.</p>
              : (
                <div className="space-y-2">
                  {[...todos].sort((a, b) => Number(a.done) - Number(b.done)).map(todo => (
                    <div key={todo.id} className={`flex items-center gap-3 p-3 rounded-xl border transition-all ${todo.done ? "border-zinc-800/40 bg-zinc-900/20 opacity-50" : "border-zinc-800 bg-zinc-900/40"}`}>
                      ...
                      <button onClick={() => deleteTodo(todo.id)} className="text-zinc-700 hover:text-red-400 transition-colors shrink-0">
                        <Trash2 size={13} />
                      </button>
                    </div>
                  ))}
        ...
        {activeTab === "objectifs" && (
          <div className="space-y-4">
            <div className="flex items-center justify-between">
              <h2 className="text-white font-bold">🎯 Objectifs</h2>
              <button onClick={() => setShowObjForm(p => !p)}
                className="flex items-center gap-1.5 px-3 py-1.5 rounded-xl text-xs font-bold border border-zinc-800 text-zinc-400 hover:text-white transition-colors">
                <Plus size={11} /> Nouvel objectif
              </button>
            </div>
            {showObjForm && (
              <div className="p-4 rounded-2xl border border-zinc-700 bg-zinc-900/60 space-y-2">
                <input value={newObjLabel} onChange={e => setNewObjLabel(e.target.value)} placeholder="Libellé (ex: CA mensuel)"
                ...
                <div className="flex gap-2">
                  <input value={newObjCible} onChange={e => setNewObjCible(e.target.value)} placeholder="Cible (ex: 5000)" type="number"
                    className="flex-1 bg-zinc-800 border border-zinc-700 rounded-xl px-3 py-2 text-sm text-white placeholder-zinc-600 focus:outline-none" />
                  <input value={newObjUnite} onChange={e => setNewObjUnite(e.target.value)} placeholder="Unité (€, %...)"
                    className="w-28 bg-zinc-800 border border-zinc-700 rounded-xl px-3 py-2 text-sm text-white placeholder-zinc-600 focus:outline-none" />
                  <button onClick={addObjectif} className="px-4 py-2 rounded-xl text-sm font-bold text-black shrink-0"
                    style={{ backgroundColor: avatarBg }}>Ajouter</button>
                </div>
              </div>
            )}
            {objectifs.length === 0
              ? <p className="text-zinc-600 text-sm italic py-8 text-center">Aucun objectif défini.</p>
              : (
                <div className="space-y-3">
                  {objectifs.map(obj => {
                      ...
                      <div key={obj.id} className="p-4 rounded-2xl border border-zinc-800 bg-zinc-900/40">
                        <div className="flex items-center justify-between mb-3">
                          ...
                          <div className="flex items-center gap-2">
                            ...
                            <button onClick={() => deleteObjectif(obj.id)} className="text-zinc-700 hover:text-red-400"><Trash2 size={12} /></button>
                          </div>
                        </div>
                        ...
                        <div className="flex items-center justify-between">
                          ...
                          <div className="flex items-center gap-1">
                            <input type="number" defaultValue={obj.valeur_actuelle}
                              onBlur={e => updateObjectifVal(obj.id, Number(e.target.value))}
                              className="w-20 bg-zinc-800 border border-zinc-700 rounded-lg px-2 py-1 text-xs text-white text-right focus:outline-none" />
        ...
        {activeTab === "postits" && (
          <div className="space-y-4">
            ...
            <div className="space-y-2">
              <textarea value={newPostitText} onChange={e => setNewPostitText(e.target.value)} rows={2}
              ...
              <div className="flex gap-2 flex-wrap items-center">
                ...
                <span className="text-zinc-600 text-xs ml-1">← Choisir la couleur</span>
              </div>
            </div>
            {postits.length === 0
              ? <p className="text-zinc-600 text-sm italic py-8 text-center">Aucun post-it.</p>
              : (
                <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 gap-3">
                  {postits.map(p => (
                    <div key={p.id} className="relative rounded-2xl p-4 shadow-lg" style={{ backgroundColor: p.color, minHeight: "120px" }}>
                      ...
                      <button onClick={() => deletePostit(p.id)}
                        className="absolute top-2 right-2 w-6 h-6 rounded-full bg-black/15 hover:bg-black/30 flex items-center justify-center transition-colors">
                        <Trash2 size={11} className="text-zinc-800" />
                      </button>
                      <p className="text-zinc-500 text-[10px] absolute bottom-2 left-4">
