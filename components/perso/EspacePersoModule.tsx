...
interface Props { activeSociety: any; profile: any; targetNom: string }
...
export default function EspacePersoModule({ activeSociety, profile, targetNom }: Props) {
  const [targetProfile, setTargetProfile] = useState<any>(null)
  const [loading, setLoading]       = useState(true)
  const [saving, setSaving]         = useState(false)
  const isOwner = (profile?.nom?.toLowerCase().includes(targetNom.toLowerCase()) ||
                   profile?.prenom?.toLowerCase().includes(targetNom.toLowerCase()))
  ...
  const load = useCallback(async () => {
    ...
    const { data: profiles } = await supabase.from("profiles")
      .select("id,nom,prenom,avatar_url,color")
      .eq("society_id", sid)
    const tp = profiles?.find(p =>
      p.nom?.toLowerCase().includes(targetNom.toLowerCase()) ||
      p.prenom?.toLowerCase().includes(targetNom.toLowerCase())
    )
