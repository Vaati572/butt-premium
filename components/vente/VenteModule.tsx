...
import {
  ShoppingCart, Plus, X, Trash2, Search, User,
    ...
    <div className="fixed inset-0 bg-black/60 backdrop-blur-sm flex justify-end z-50">
      <div className="bg-[#111111] border-l border-zinc-800 w-full max-w-lg h-full flex flex-col shadow-2xl">
        ...
        <div className="flex-1 overflow-y-auto p-4 space-y-2">
          ...
          : ventes.length === 0 ? <div className="text-center py-12 text-zinc-600"><Receipt size={32} className="mx-auto mb-3 opacity-20" /><p className="text-sm">Aucune vente</p></div>
          : ventes.map(v => (
...
function AddProductPanel({ societyId, onClose, onDone }: { societyId: string; onClose: () => void; onDone: () => void }) {
  ...
  const handleSave = async () => {
    ...
    await supabase.from("products").insert({
      society_id: societyId, name: nom, pv: parseFloat(pv) || 0,
      cf: parseFloat(cf) || 0, gamme, in_stock: true,
    })
    setSaving(false); onDone(); onClose()
  }
  return (
...
export default function VenteModule({ activeSociety, profile }: Props) {
  ...
  const filteredProducts = activeGamme
    ? products.filter(p => {
        ...
        const matchGamme = p.gamme === activeGamme
      ...
      for (const item of cart) {
        ...
        if (fullProduct?.composition) {
          if (typeof fullProduct.composition === "string") {
            try { composition = JSON.parse(fullProduct.composition) } catch {}
          } else {
        ...
        if (compoEntries.length > 0) {
          ...
          if (stockItem) {
            ...
            await supabase.from("stock_history").insert({
              ...
              quantite_avant: stockItem.quantite,
              quantite_apres: newQty,
              notes: `Vente — ${selectedClient?.nom || "passage"}`,
            })
            stockItem.quantite = newQty
          }
        }
  ...
  return (
    <div className="flex-1 flex overflow-hidden bg-[#0a0a0a]">
      ...
      {/* ── DROITE : Panier ────────────────────── */}
      <div className="w-96 bg-[#111111] flex flex-col overflow-hidden">
          ...
          <div className="border-t border-zinc-800 p-4 space-y-3">
            <div className="flex gap-2">
              ...
              <div className="w-24">
                ...
                <input type="text" placeholder="0.00" value={portPerso} onChange={(e) => setPortPerso(e.target.value)}
                  className="w-full bg-zinc-800 border border-zinc-700 rounded-lg px-2 py-1.5 text-sm text-white text-center focus:outline-none" />
              </div>
            </div>

            <div>
              <label className="block text-[10px] text-zinc-500 font-semibold uppercase tracking-wider mb-1">NOTES</label>
              <input type="text" placeholder="Notes de la vente..." value={notes} onChange={(e) => setNotes(e.target.value)}
                className="w-full bg-zinc-800 border border-zinc-700 rounded-lg px-3 py-1.5 text-sm text-white placeholder-zinc-600 focus:outline-none" />
            </div>
            ...
            <button onClick={handleVente} disabled={saving || cart.length === 0}
              ...
              {success ? <><Check size={16} /> ✔ Vente enregistrée !</>
                ...
                : <><ShoppingCart size={15} /> ✔ VALIDER LA VENTE — {totalTTC.toFixed(2)}€</>}
            </button>
          </div>
        )}
      </div>
