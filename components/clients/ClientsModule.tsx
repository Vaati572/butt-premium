...
import {
  ...
  Pencil, Trash2, Tag, Calendar,
  Euro, Check, TrendingUp
} from "lucide-react"
...
const CONTRAT_TIERS: { label: string; cardColor: string | null }[] = [
  ...
  { label: "Essentielle", cardColor: "#cd7f32" },
  { label: "Avantage",    cardColor: "#94a3b8" },
  { label: "Élite",       cardColor: "#eab308" },
  { label: "ProTeam",     cardColor: "#a855f7" },
]
...
function TarifsPanel({ client, societyId, onClose }: { client: Client; societyId: string; onClose: () => void }) {
  ...
  const saveTarifs = async () => {
    ...
    const rows = Object.entries(prixMap).filter(([_, prix]) => prix > 0).map(([product_id, prix]) => ({ client_id: client.id, product_id, prix }))
    ...
    <div className="fixed inset-0 bg-black/60 backdrop-blur-sm flex justify-end z-50">
      <div className="bg-[#111111] border-l border-zinc-800 w-full max-w-lg h-full flex flex-col shadow-2xl">
        ...
        <div className="px-5 py-3 border-b border-zinc-800 flex items-center gap-3">
          ...
          <button onClick={applyBaseGamme} className="flex items-center gap-1 text-xs text-blue-400 bg-blue-400/10 border border-blue-400/20 px-2.5 py-1.5 rounded-lg hover:bg-blue-400/20">↩ Appliquer</button>
        </div>
...
function GeolocButton({ adresse, cp, ville, onLocate, hasCoords, lat, lng }: {
  ...
  const geocode = async () => {
    ...
    } catch { setError("Erreur de géolocalisation") }
    setLoading(false)
  }
    ...
    <div className="mt-2">
      <div className="flex items-center gap-2">
        <button type="button" onClick={geocode} disabled={loading}
          ...
          {loading ? <><div className="w-3 h-3 border-2 border-blue-400 border-t-transparent rounded-full animate-spin"/>Localisation...</> : <>📍 Géolocaliser l'adresse</>}
        </button>
        {hasCoords && lat && lng && (
...
function ClientForm({ societyId, profile, client, onClose, onDone }: {
  ...
  const [form, setForm] = useState({
    ...
    nom_shop:  client?.nom_shop || "",
    email:     client?.email || "",
    telephone: client?.telephone || "",
    adresse:   client?.adresse || "",
    ville:     client?.ville || "",
    cp:        client?.cp || "",
    contrat:   client?.contrat || "Aucun",
    ...
    <div className="fixed inset-0 bg-black/60 backdrop-blur-sm flex justify-end z-50">
      <div className="bg-[#111111] border-l border-zinc-800 w-full max-w-md h-full flex flex-col shadow-2xl">
        ...
        <div className="flex-1 overflow-y-auto p-5 space-y-4">
          ...
          {/* Nom du shop */}
          <div>
            <label className="block text-[11px] font-semibold text-zinc-500 uppercase tracking-wider mb-1.5">🏪 Nom du shop / salon</label>
            <input value={form.nom_shop} onChange={e => set("nom_shop", e.target.value)} placeholder="Ex: Studio Ink, Salon Beauty..."
...
function ClientCard({ client, accentColor, onEdit, onDelete, onTarifs }: {
  ...
  accentColor: string; onEdit: () => void; onDelete: () => void; onTarifs: () => void
}) {
  ...
  return (
    <div className="group relative rounded-2xl overflow-hidden transition-all duration-200 hover:shadow-xl flex flex-col"
      style={{
        ...
        border: hasBorderColor ? `1px solid ${cardBorderColor}70` : `1px solid #27272a`,
        boxShadow: hasBorderColor ? `0 0 24px ${cardBorderColor}25` : undefined,
      }}>

      {hasBorderColor && <div className="h-1 w-full" style={{ backgroundColor: cardBorderColor }}/>}
      ...
      <div className="flex items-center gap-3 px-4 pt-4 pb-3">
        <div className="relative shrink-0">
          <div className="w-11 h-11 rounded-xl overflow-hidden flex items-center justify-center text-white font-black text-sm ring-2 ring-black"
            ...
            {client.avatar_url ? <img src={client.avatar_url} alt={client.nom} className="w-full h-full object-cover"/> : initials(client.nom)}
          </div>
          <span className={`absolute -bottom-0.5 -right-0.5 w-3 h-3 rounded-full ring-2 ring-[#111111] ${
            client.statut === "vip" ? "bg-yellow-400" : client.statut === "inactif" ? "bg-zinc-600" : "bg-green-400"
          }`}/>
        </div>
        ...
        <div className="flex-1 min-w-0">
          ...
          {/* Nom du shop */}
          {client.nom_shop && (
            <p className="text-zinc-400 text-[11px] font-medium truncate mt-0.5">🏪 {client.nom_shop}</p>
          )}
          <div className="flex items-center gap-1.5 mt-0.5 flex-wrap">
            ...
            {(client.tags || []).slice(0, 2).map(tag => (
              <span key={tag} className="text-[10px] font-semibold px-1.5 py-0.5 rounded-md"
                style={{ color: accentColor, backgroundColor: accentColor+"18" }}>{tag}</span>
            ))}
          </div>
        </div>

        <div className="flex items-center gap-0.5 shrink-0">
          <button onClick={onTarifs} className="w-7 h-7 flex items-center justify-center text-blue-400 hover:text-blue-300 rounded-lg hover:bg-blue-400/10" title="Tarifs perso">
            <Euro size={13}/>
          </button>
          <button onClick={onEdit} className="w-7 h-7 flex items-center justify-center text-zinc-500 hover:text-white rounded-lg hover:bg-zinc-700">
            <Pencil size={13}/>
          </button>
          <button onClick={onDelete} className="w-7 h-7 flex items-center justify-center text-zinc-700 hover:text-red-400 rounded-lg hover:bg-red-500/10">
            <Trash2 size={13}/>
          </button>
        </div>
      </div>
      ...
      <div className="px-4 py-3 space-y-1.5 flex-1">
        {client.telephone && (
          <a href={`tel:${client.telephone}`} className="flex items-center gap-2.5 group/link">
            <div className="w-5 h-5 rounded-md bg-zinc-800 flex items-center justify-center shrink-0"><Phone size={10} className="text-zinc-400"/></div>
            <span className="text-zinc-300 text-xs font-medium group-hover/link:text-white transition-colors">{client.telephone}</span>
          </a>
        )}
        {client.email && (
          <a href={`mailto:${client.email}`} className="flex items-center gap-2.5 group/link">
            <div className="w-5 h-5 rounded-md bg-zinc-800 flex items-center justify-center shrink-0"><Mail size={10} className="text-zinc-400"/></div>
            <span className="text-zinc-400 text-xs truncate group-hover/link:text-white transition-colors">{client.email}</span>
          </a>
        )}
        {(client.adresse || client.ville) && (
          <div className="flex items-start gap-2.5">
            <div className="w-5 h-5 rounded-md bg-zinc-800 flex items-center justify-center shrink-0 mt-0.5"><MapPin size={10} className="text-zinc-400"/></div>
        ...
        {client.notes && (
          <div className="flex items-start gap-2.5 pt-0.5">
            <div className="w-5 h-5 rounded-md bg-zinc-800/60 flex items-center justify-center shrink-0 mt-0.5"><span className="text-[9px]">📝</span></div>
            <p className="text-zinc-500 text-[11px] italic leading-relaxed line-clamp-2">{client.notes}</p>
          </div>
        )}
      </div>
      ...
      <div className="px-4 py-2.5 flex items-center justify-between">
        <div className="flex items-center gap-1.5">
          ...
          {(client.nb_achats || 0) > 0 && (
            <><span className="text-zinc-700 text-[10px]">·</span><span className="text-zinc-500 text-[10px]">{client.nb_achats} achat{(client.nb_achats||0)>1?"s":""}</span></>
          )}
        </div>
        {client.created_at && (
          <div className="flex items-center gap-1">
            <Calendar size={10} className="text-zinc-700"/>
            <span className="text-zinc-700 text-[10px]">{new Date(client.created_at).toLocaleDateString("fr-FR", { month:"short", year:"numeric" })}</span>
          </div>
        )}
      </div>
    </div>
...
export default function ClientsModule({ activeSociety, profile }: Props) {
  ...
  const load = useCallback(async () => {
    ...
    setLoading(true)
    const { data: cls } = await supabase.from("clients").select("*").eq("society_id", activeSociety.id).order("nom")
    if (!cls) { setLoading(false); return }
    const { data: ventes } = await supabase.from("ventes").select("client_id, total_ttc").eq("society_id", activeSociety.id).not("client_id", "is", null)
    ...
    setClients(cls.map(c => ({ ...c, ca_total: caMap[c.id]?.total || 0, nb_achats: caMap[c.id]?.count || 0 })))
    setLoading(false)
  }, [activeSociety?.id])
      ...
      <div className="p-6 max-w-6xl mx-auto">
        ...
        <div className="flex items-center gap-2 mb-5 flex-wrap">
          <div className="relative flex-1 min-w-[180px]">
            ...
            <input type="text" placeholder="Nom, shop, email, ville, tag..." value={search} onChange={e => setSearch(e.target.value)}
          ...
          <div className="flex items-center gap-1 bg-zinc-900 border border-zinc-800 rounded-xl p-1">
            {([["nom","A-Z"],["ca","CA ↓"],["date","Date"]] as const).map(([val,lbl])=>(
              <button key={val} onClick={()=>setSortBy(val as any)}
                className={`px-3 py-1.5 rounded-lg text-xs font-semibold transition-colors ${sortBy===val?"bg-zinc-700 text-white":"text-zinc-500 hover:text-zinc-300"}`}>{lbl}</button>
            ))}
          </div>
        </div>
        ...
        {loading ? (
          ...
          <div className="text-center py-24 text-zinc-600">
            ...
            {!search && (
              <button onClick={() => setShowForm(true)} className="mt-4 bg-yellow-500 hover:bg-yellow-400 text-black font-bold px-5 py-2.5 rounded-xl text-sm inline-flex items-center gap-2">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-4">
            {filtered.map(client => (
              <ClientCard key={client.id} client={client} accentColor={accentColor}
