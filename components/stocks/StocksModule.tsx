...
export default function StocksModule({ activeSociety, profile }: Props) {
  ...
  const filteredSorted = visibleStock
    .filter(item => {
  ...
  const valeurTotale = visibleStock.reduce((s,i)=>s+((i.quantite>0&&i.prix_achat)?i.quantite*i.prix_achat:0),0)
      ...
      <div className="p-6 max-w-7xl mx-auto">
        ...
        {loading ? (
          ...
          <div className="text-center py-24 text-zinc-600">
            ...
            {!search && <button onClick={()=>setShowAdd(true)} className="mt-4 text-yellow-500 text-xs hover:underline">+ Ajouter</button>}
          </div>
        ) : viewMode==="table" ? (
