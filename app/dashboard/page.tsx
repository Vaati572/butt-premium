...
function InnerDashboard({ profile, activeSociety }: { profile: any; activeSociety: any }) {
  ...
  /* ── Rendu grande case nav — section "Activité" mise en avant ── */
  const renderPrimaryNavItem = (tab: any) => {
    ...
    return (
      <button key={tab.id}
        ...
        style={{
          backgroundColor: isActive ? ACCENT + "1f" : "rgba(255,255,255,0.04)",
          border: isActive ? `1.5px solid ${ACCENT}80` : "1.5px solid rgba(255,255,255,0.06)",
          boxShadow: isActive ? `0 2px 20px ${ACCENT}25` : "none",
        }}
        onMouseEnter={e => {
          if (!isActive) {
            const el = e.currentTarget as HTMLElement
        ...
        {isActive && (
          <span className="absolute left-0 top-1/2 -translate-y-1/2 w-1 h-7 rounded-full"
            style={{ backgroundColor: ACCENT, boxShadow: `0 0 8px ${ACCENT}` }} />
        )}
        <div className="w-10 h-10 rounded-xl flex items-center justify-center text-lg shrink-0 transition-colors"
          style={{ backgroundColor: isActive ? ACCENT + "30" : "rgba(255,255,255,0.06)" }}>
          {tab.icon}
        </div>
        <span className="flex-1 text-left text-[14px] font-bold truncate" style={{ color: isActive ? "#ffffff" : "#d4d4d8" }}>
          {tab.label}
        </span>
        {isOpen && !isActive && (
          <span className="w-1.5 h-1.5 rounded-full shrink-0" style={{ backgroundColor: ACCENT + "80" }}/>
        ...
        {tab.id === "messages" && unreadMessages > 0 && (
          <span className="text-black text-[10px] font-black min-w-[18px] h-[18px] px-1 rounded-full flex items-center justify-center shrink-0"
            style={{ backgroundColor: ACCENT }}>{unreadMessages > 9 ? "9+" : unreadMessages}</span>
        )}
      </button>
    )
  ...
  return (
    <div className="h-screen text-white flex overflow-hidden"
      ...
      {/* ══ SIDEBAR DESKTOP ══ */}
      <aside className="hidden md:flex w-64 border-r border-zinc-900 flex-col shrink-0 h-screen overflow-hidden"
